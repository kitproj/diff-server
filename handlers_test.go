...
func TestSignalHandling(t *testing.T) {
	// Build the binary
	buildCmd := exec.Command("go", "build", "-o", "test-diffs-cli", ".")
	if err := buildCmd.Run(); err != nil {
		t.Fatalf("failed to build server: %v", err)
	}
	defer os.Remove("test-diffs-cli")

	// Start the server in a subprocess
	cmd := exec.Command("./test-diffs-cli", "-port", "8892")
	...
	// The important thing is that the server shuts down gracefully
	// within a reasonable time, not the exit code
	select {
	case <-done:
		// Server exited successfully (graceful shutdown)
	case <-time.After(5 * time.Second):
		cmd.Process.Kill()
		t.Fatal("server did not shut down gracefully within 5 seconds after receiving SIGINT")
	}
}
